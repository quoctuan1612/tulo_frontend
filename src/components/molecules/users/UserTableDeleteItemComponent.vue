<template>
  <v-dialog v-model="value" max-width="500px" persistent>
    <v-card>
      ...
      >
        <span class="title-form">Xóa tài khoản</span>
      </v-toolbar>

      <v-card-text>
        Bạn có chắc chắn muốn xóa thông tin tài khoản này không?
      </v-card-text>
...
export default {
   ...
   */
  methods: {
    ...mapActions("common", ["delete_user", "get_users"]),
    ...
    async delete() {
      try {
        await this.delete_user(this.user_data.id);
